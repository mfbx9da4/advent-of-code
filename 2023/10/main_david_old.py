...
start_cell = next(Cell(r, c) for r, row in enumerate(board)
                  for c, column in enumerate(row) if column == "S")


def move(cell: Cell, move_name: Optional[str]) -> Optional[Cell]:
    moves = {
        #       r, c
        "left": (0, -1),
        "right": (0, 1),
    ...
    if move_name is None:
        return None
    move_row, move_col = moves[move_name]
    next_cell = Cell(
        cell.row + move_row, cell.col + move_col)
...
while len(nodes) > 0:
    ...
    move_str = pipes[char].get(node.prev_direction)
    if move_str is not None:
        seen.add(node.cell)
        next_cell = move(node.cell, move_str)
        if next_cell:
            nodes.append(Node(next_cell, node.distance + 1, move_str))
...
# 2. Find cells connected to the wall and not to the path
...
def find_path_from_start():
    for direction in ["up", "right", "down", "left"]:
        node = Node(start_cell, 0, None)
        path = [start_cell]
        while True:
            char = board[node.cell.row][node.cell.col]
            move_str = direction if char == "S" else pipes[char].get(
                node.prev_direction)
            next_cell = move(node.cell, move_str)
            if not next_cell:
                # no loop
            ...
            node = Node(next_cell, node.distance + 1, move_str)
...
for r, row in enumerate(board):
    for c, cell in enumerate(row):
        ...
        if c == 0 or c == len(row) - 1 or r == 0 or r == len(board) - 1:
            first_outsiders.append(cell)
            break
assert first_outsiders is not None

# 3. Flood fill from those outsider cells
outiders = set(first_outsiders)
open_list = first_outsiders
while len(open_list) > 0:
    cell = open_list.pop(0)
    for direction in ["up", "right", "down", "left"]:
        next_cell = move(cell, direction)
        if next_cell and next_cell not in outiders and next_cell not in path:
            outiders.add(next_cell)
            open_list.append(next_cell)

# 4. Now find a cell on the path connected empty on one side and flood fill on the other
first_insiders = []
for r, row in enumerate(board):
    for c, cell in enumerate(row):
        cell = Cell(r, c)
        ...
        # this cell is either an insider or enclosed outsider
        # insiders will have up(1) = path, up(2) = outsider
        up = move(cell, "up")
        up2 = move(up, "up") if up else None
        if up and up2 and up in path and up2 in outiders:
            first_insiders.append(cell)
            continue

        down = move(cell, "down")
        down2 = move(down, "down") if down else None
        if down and down2 and down in path and down2 in outiders:
            first_insiders.append(cell)
            continue

        right = move(cell, "right")
        right2 = move(right, "right") if right else None
        if right and right2 and right in path and right2 in outiders:
            first_insiders.append(cell)
            continue

        left = move(cell, "left")
        left2 = move(left, "left") if left else None
        if left and left2 and left in path and left2 in outiders:
            first_insiders.append(cell)
            continue


assert first_insiders is not None


# 5. Fill the inside -> answer!
insiders = set([*first_insiders, *path])
open_list = first_insiders
while len(open_list) > 0:
    cell = open_list.pop(0)
    for direction in ["up", "right", "down", "left"]:
        next_cell = move(cell, direction)
        if next_cell and next_cell not in insiders and next_cell not in outiders:
            insiders.add(next_cell)
            open_list.append(next_cell)


# Print the board
print("Board")
[print("".join([str(c) for c in row])) for row in board]
# Print the first insiders
print("First insiders")
[print("".join(['X' if Cell(r, c) in first_insiders else '.' for c, _ in enumerate(row)]))
 for r, row in enumerate(board)]
# Print the outsiders
print("Outsiders")
[print("".join([str('X' if Cell(r, c) in outiders else '.') for c, _ in enumerate(row)]))
 for r, row in enumerate(board)]
print("Insiders")
[print("".join([str('X' if Cell(r, c) in insiders else '.') for c, _ in enumerate(row)]))
 for r, row in enumerate(board)]

print("Part 2 answer:", len(insiders) - len(path))
