...
function move(cell, direction) {
  const { r, c } = cell;
  let nextCell = null;
  if (direction === "left") nextCell = { r, c: c - 1 };
  if (direction === "right") nextCell = { r, c: c + 1 };
  if (direction === "up") nextCell = { r: r - 1, c };
  if (direction === "down") nextCell = { r: r + 1, c };
  return nextCell && inBounds(nextCell) ? nextCell : null;
}

function findPath() {
  for (const startDirection of ["left", "right", "up"]) {
    let prevDirection = null;
    let node = {
      ...startCell,
      directionOut: startDirection,
    };
    const path = [node];
    while (true) {
      const char = board[node.r][node.c];
      const direction =
        char === "S" ? startDirection : pipes[char][prevDirection];
      node.directionOut = direction;
      const cell = move(node, direction);
      if (!cell) break;
      node = { ...cell, directionOut: null };
      prevDirection = direction;

      if (node.r === startCell.r && node.c === startCell.c) {
        return path;
      } else {
        path.push(node);
      }
...
const path = findPath();
// Workout which pipe the start cell is
const [startNode] = path;
const prevDirection = path[path.length - 1].directionOut;
const directionOut = path[0].directionOut;
const startPipe =
  prevDirection === "right" && directionOut === "right"
    ? "-"
    : prevDirection === "right" && directionOut === "down"
    ? "7"
    : prevDirection === "right" && directionOut === "up"
    ? "J"
    : prevDirection === "left" && directionOut === "left"
    ? "-"
    : prevDirection === "left" && directionOut === "up"
    ? "L"
    : prevDirection === "left" && directionOut === "down"
    ? "7"
    : prevDirection === "down" && directionOut === "down"
    ? "|"
    : prevDirection === "down" && directionOut === "left"
    ? "J"
    : prevDirection === "down" && directionOut === "right"
    ? "L"
    : prevDirection === "up" && directionOut === "up"
    ? "|"
    : prevDirection === "up" && directionOut === "left"
    ? "7"
    : prevDirection === "up" && directionOut === "right"
    ? "F"
    : null;

console.log("startPipe", startPipe, prevDirection, directionOut);

function getChar(cell) {
  const { r, c } = cell;
  let char = board[r][c];
  return char === "S" ? startPipe : char;
}

function labelInnerAndOuterCells() {
  // 1. Iterate first column and bottom row
  // 2. Iterate diagonally, starting in state outside
  const labelled = [];
  const startCells = [];
  for (let r = 0; r < board.length; r++) {
    startCells.push({ r, c: 0 });
  }
  for (let c = 0; c < board[0].length; c++) {
    startCells.push({ r: board.length - 1, c });
  }
  const isPathCell = (cell) =>
    path.find((node) => node.r === cell.r && node.c === cell.c);

  for (const startCell of startCells) {
    let cell = startCell;
    let state = isPathCell(cell) ? "inside" : "outside";
    while (true) {
      if (!isPathCell(cell)) {
        labelled.push({ ...cell, state });
      }
      cell = move(cell, "right");
      cell = cell ? move(cell, "up") : null;
      if (!cell) break;
      if (isPathCell(cell) && getChar(cell) !== "F" && getChar(cell) !== "J") {
...
const labelled = labelInnerAndOuterCells();

function drawBoard() {
  let innerHTML = "";
  for (let r = 0; r < board.length; r++) {
    innerHTML += '<div class="row" style="display: flex;">';
    for (let c = 0; c < board[r].length; c++) {
      const char = board[r][c];
      innerHTML += /*html*/ `<div class="cell"
        style="
          width: 20px;
          height: 20px;
...
async function drawPath() {
  ...
  for (const [i, node] of path.entries()) {
    const cellElement = rowElements[node.r].children[node.c];
    const green = (i / path.length) * 155 + 100;
    cellElement.style.backgroundColor = `rgb(0, ${green}, 0)`;
    cellElement.style.color = "white";
    cellElement.style.fontWeight = "bold";
    cellElement.style.position = "relative";
    // Draw arrow directionIn and directionOut
    ...
    arrowElement.style.color = "silver";
    cellElement.appendChild(arrowElement);
    if (node.directionOut === "right") {
      arrowElement.className = "right";
      arrowElement.style.transform = "rotate(90deg)";
      arrowElement.style.top = "4px";
      arrowElement.style.right = "-12px";
    }
    if (node.directionOut === "down") {
      arrowElement.className = "down";
      arrowElement.style.transform = "rotate(180deg)";
      arrowElement.style.bottom = "-14px";
      arrowElement.style.left = "7px";
    }
    if (node.directionOut === "left") {
      arrowElement.className = "left";
      arrowElement.style.transform = "rotate(270deg)";
      arrowElement.style.top = "4px";
      arrowElement.style.left = "-12px";
    }
    if (node.directionOut === "up") {
      arrowElement.className = "up";
      arrowElement.style.top = "-14px";
      arrowElement.style.left = "7px";
    }
...
async function drawInsidersAndOutsiders() {
  const rowElements = document.querySelectorAll(".row");
  for (const cell of labelled) {
    const cellElement = rowElements[cell.r].children[cell.c];
    cellElement.style.backgroundColor =
      cell.state === "inside" ? "blue" : "red";
    cellElement.style.color = "white";
