...
# 1. Loop through rowXcol
# 2. Keep track of inside or outside
# 3. Start -1, -1 so we know we're outside?
...
for r in range(len(board)):
    for c in range(len(board[r])):
        ...
        if cell in path_cells:
            ...
            # If we've walked along the path
            # Otherwise if we've traversed a wall flip the inside/outside
            up = move(cell, "up")
            if up and up in insiders:
                insiders.add(cell)
            else:
                outsiders.add(cell)
...
[print("".join(['X' if Cell(r, c) in outsiders else '.' for c, _ in enumerate(row)]))
 for r, row in enumerate(board)]
print("Insiders")
[print("".join(['X' if Cell(r, c) in insiders else '.' for c, _ in enumerate(row)]))
 for r, row in enumerate(board)]
