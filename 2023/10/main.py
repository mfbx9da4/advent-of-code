...
# 2. open_list = [start]
# 3. BFS -> until cycle
# 4. return the distance of cycle
...
for r, row in enumerate(board):
    for c, col in enumerate(row):
        if col == "S":
            start_coordinate = {"row": r, "column": c}
            break
...
for direction in ["up", "right", "down", "left"]:
    ...
    while True:
        ...
        if coordinate["row"] < 0 or coordinate["column"] < 0 or coordinate["row"] >= len(board) or coordinate["column"] >= len(board[0]):
            break

        pipe = board[coordinate["row"]][coordinate["column"]]

        if pipe == "S":
            maximum_cycle_length = max(maximum_cycle_length, cycle_length)
            maximum_cycle_path = path
            break
        elif pipe == "|" and direction == "down":
            direction = "down"
        elif pipe == "|" and direction == "up":
            direction = "up"
        elif pipe == "-" and direction == "right":
            direction = "right"
        elif pipe == "-" and direction == "left":
            direction = "left"
        elif pipe == "F" and direction == "up":
            direction = "right"
        elif pipe == "F" and direction == "left":
            direction = "down"
        elif pipe == "J" and direction == "right":
            direction = "up"
        elif pipe == "J" and direction == "down":
            direction = "left"
        elif pipe == "L" and direction == "down":
            direction = "right"
        elif pipe == "L" and direction == "left":
            direction = "up"
        elif pipe == "7" and direction == "right":
            direction = "down"
        elif pipe == "7" and direction == "up":
            direction = "left"
        else:
            break

print(maximum_cycle_length//2)
# print(maximum_cycle_path)
...
for r, row in enumerate(board):
    for c, col in enumerate(row):
        if {"row": r, "column": c} in maximum_cycle_path:
            continue
        ...
        pipes_count = 0
        for _r in range(0, r):
            if {"row": _r, "column": c} in maximum_cycle_path and not board[_r][c] in ["|", "7", "F"]:
                pipes_count += 1
        if pipes_count % 2 == 0:
            continue
        ...
        # right
        pipes_count = 0
        for _c in range(c + 1, len(board[r])):
            if {"row": r, "column": _c} in maximum_cycle_path and not board[r][_c] in ["-", "J", "7"]:
                pipes_count += 1
        if pipes_count % 2 == 0:
            continue
...
# ISSUE 1: S works as 7, F, and so forth
# ISSUE 2: Off-by-one somewhere
