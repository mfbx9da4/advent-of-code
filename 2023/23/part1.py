...
while open_list:
    ...
    if board[r][c] != "." and board[r][c] != node.direction:
        continue  # Can't go through

    if node.coords in node.path:
        continue  # Avoid loops

    if r == len(board) - 1:
        print("Found path", len(node.path), max(max_len, len(node.path)))
