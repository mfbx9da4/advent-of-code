...
# ------------------------|---------------|---------------| ...
# ---|--------|--------|--------|--------|--------|--------| ...
# Cycles repeat at cycle_offset + (cycle_length * i) + end_node_offset_within_cycle
# Minor simplification we can ignore end_node_offset_within_cycle and add it back later
# formula is cycle_offset + (cycle_length * i) or o + l * i
# what is the i such that o1 + (l1 * i) == o2 + (l2 * i) == ... on + (ln * i)
# if none had any offsets then it would be simply l1 * i  == l2 * i == ... ln * i

for data in cycle_data:
    end_nodes = set()
    for i in range(1000000000):
        end_node = data["cycle_offset"] + \
            ((data["cycle_length"] * i) + data["end_node_offset_within_cycle"])
        if all([end_node in x for x in ends_nodes]):
            end_nodes.add(end_node)
    ends_nodes.append(end_nodes)

part2_answer = float('inf')
for combination in itertools.product(*[list(x) for x in ends_nodes]):
    if all([x == combination[0] for x in combination]):
        if combination[0] < part2_answer:
            part2_answer = combination[0]
            print(combination)
